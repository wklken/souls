...
The generated SOUL.md must pass markdownlint. Follow these rules strictly:

1. **Blank lines around headings (MD022)**: Every heading (`##`, `###`) must have a blank line both above AND below it before any content starts
2. **Blank lines around lists (MD032)**: Every list block (lines starting with `-`) must have a blank line before the first item and after the last item
3. **No emphasis as heading (MD036)**: Never use a standalone `**bold text**` line as a pseudo-heading. Use a blockquote (`> text`) or a proper heading (`###`) instead
4. **Table separator spacing (MD060)**: Table separator rows must match the header style with spaces around pipes: `| ------ | --------- |` not `|------|---------|`
5. **Consistent punctuation**: Use Chinese commas (，) between Chinese text items and English commas (,) between English text items — do not mix them
